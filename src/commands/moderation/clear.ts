...
import type { ClearCommand, ClearMessageContextCommand } from "../../interactions/index.js";
...
@injectable()
export default class extends Command<typeof ClearCommand | typeof ClearMessageContextCommand> {
	...
	private async handle(
		...
		if (lastMessage && firstMessage.channelId !== interaction.channelId) {
			throw new Error(
		...
		if (collectedInteraction?.customId === cancelKey) {
			await collectedInteraction.update({
				content: i18next.t("command.mod.clear.cancel", { lng: locale }),
				components: [],
				embeds: [],
			});
			...
			try {
				...
				const embed = addFields({
					...
					title: i18next.t("log.guild_log.messages_cleared.title", { lng: locale }),
	...
	public override async messageContext(
		...
		args: ArgsParam<typeof ClearMessageContextCommand>,
		locale: LocaleParam,
	): Promise<void> {
