...
type CaseFooter = {
	...
	rerstriction?: number | undefined;
	timeout?: number | undefined;
	warn?: number | undefined;
};

type ValuesTuple = [number, number, number, number, number, number];
...
function generateHistoryEmbed(
	...
	const truncatedPhrase = i18next.t("common.and_more", { lng: locale });
	...
	for (const record of records) {
		...
		if (
			...
			EMBED_DESCRIPTION_LIMIT - truncatedPhrase.length
		) {
			descriptionParts.push(recordString);
		} else {
			descriptionParts.push(truncatedPhrase);
		}
	}
...
export async function generateCaseHistory(
	interaction: ButtonInteraction<"cached"> | CommandInteraction<"cached"> | AnySelectMenuInteraction<"cached">,
	...
	const records: HistoryRecord[] = cases.map((case_) => {
		return {
			...
			indentifierLabel: `#${case_.case_id}`,
			identifierURL: case_.log_message_id
				? messageLink(modLogChannelId, case_.log_message_id, interaction.guildId)
				: undefined,
...
export function generateUserInfo(target: { member?: GuildMember | undefined; user: User }, locale: string) {
	...
	if (target.member?.joinedTimestamp) {
		const sinceJoinedFormatted = time(dayjs(target.member.joinedTimestamp).unix(), TimestampStyles.RelativeTime);
		const joinedFormatted = time(dayjs(target.member.joinedTimestamp).unix(), TimestampStyles.ShortDateTime);
