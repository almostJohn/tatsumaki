...
export async function resolveMessage(
	...
	if (
		...
		channel.parentId &&
		ignoreChannels.includes(channel.parentId) &&
		channel.parent?.parentId &&
		ignoreChannels.includes(channel.parent.parentId)
	) {
		throw new Error(i18next.t("command.common.errors.ignored_channel", { lng: locale }));
	}
